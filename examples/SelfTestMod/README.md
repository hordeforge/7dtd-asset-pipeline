...
| `assets-src/bundle/shamwaySelfTestBird.glb` / `_Arachnid.glb` / `_Dino.glb` / `_Crocodile.glb` / `_Humanoid.glb` | the remaining shipped base rigs as generated entities at the quadruped construction bar (atlas, Idle1+Walk, role-aware hide) so every body plan — wings, eight legs, biped, sprawl — is a bundle member. Look suites stay per-stem; do not comma-list them with `*_block_*` |
...
| `Config/entityclasses.xml` | the six generated entity classes (creature, bird, arachnid, dino, crocodile, humanoid): mandatory `Prefab` + `Mesh`, plus `UserSpawnType="Menu"` so the console `spawnentity` command lists them, and the animated ones carry the mod-owned `Class` + `AvatarController` |
