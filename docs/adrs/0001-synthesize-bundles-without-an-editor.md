...
| material, shader | `bundle_writer.material`, `bundle_writer.shader`, `shader_blob.py` | **serialized, and drawing since 2026-08-24.** On a real graphics device `isSupported=True` and `verify-bundle --draw` reports `covered=38.8% zoomed-out=2.4%` for a prop using it. It took three fixes: an eight-byte-short GLCore code record, a missing `GL_ARB_explicit_attrib_location`, and a render state whose `name` was `""` instead of Unity's `<noninit>` sentinel, which set `colMask` to 0. The `isSupported = true` first recorded here was measured under `-nographics` and remains withdrawn — it was not evidence then, whatever is true now. **A human then looked, in a live client: invisible at first**, isolated to the d3d11 sub-program, whose constant-buffer offsets this writer had laid out differently from Unity's; fixed the same day and gated offline by `assert_cbuffer_layout`. Signed off by eye on OpenGL Core, and Direct3D 11 confirmed by eye after the fix — see [blockers.md](../status/blockers.md) |
