...
| material, shader | `bundle_writer.material`, `bundle_writer.shader`, `shader_blob.py` | **serialized, not working.** The objects load and the PPtr chain resolves; the pass does not compile on a real graphics device (`isSupported=False`), so a prop using it draws nothing. The `isSupported = true` first recorded here was measured under `-nographics` and is withdrawn |
