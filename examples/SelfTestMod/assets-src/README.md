...
| `shamwaySelfTestBird` | `shamway generate creature bundle/shamwaySelfTestBird.glb --rig bird --atlas creatures/shamwaySelfTestBird.atlas.json --hide bundle/shamwaySelfTestBird_albedo.png --coat slate --seed 7 --size 256` | bird rig, per-part UV atlas, Idle1 (bob+flap+head) + Walk on perched legs (wings are not walk legs), slate role-aware hide | `bundle/shamwaySelfTestBird.glb` + `_albedo.png` + `.anim.json` | not yet — the `--look` run is the owed picture |
| `shamwaySelfTestArachnid` | `shamway generate creature bundle/shamwaySelfTestArachnid.glb --rig arachnid --atlas creatures/shamwaySelfTestArachnid.atlas.json --hide bundle/shamwaySelfTestArachnid_albedo.png --coat charcoal --seed 7 --size 256` | arachnid rig, eight-leg tetrapod Walk, charcoal hide (tarsi painted as paws) | `bundle/shamwaySelfTestArachnid.glb` + `_albedo.png` + `.anim.json` | not yet — the `--look` run is the owed picture |
| `shamwaySelfTestDino` | `shamway generate creature bundle/shamwaySelfTestDino.glb --rig dinosaur --atlas creatures/shamwaySelfTestDino.atlas.json --hide bundle/shamwaySelfTestDino_albedo.png --coat olive --seed 7 --size 256` | dinosaur rig, biped Walk on Thighs, olive hide | `bundle/shamwaySelfTestDino.glb` + `_albedo.png` + `.anim.json` | not yet — the `--look` run is the owed picture |
| `shamwaySelfTestCrocodile` | `shamway generate creature bundle/shamwaySelfTestCrocodile.glb --rig crocodile --atlas creatures/shamwaySelfTestCrocodile.atlas.json --hide bundle/shamwaySelfTestCrocodile_albedo.png --coat rust --seed 7 --size 256` | crocodile rig, four-leg sprawl Walk, rust hide | `bundle/shamwaySelfTestCrocodile.glb` + `_albedo.png` + `.anim.json` | not yet — the `--look` run is the owed picture |
| `shamwaySelfTestHumanoid` | `shamway generate creature bundle/shamwaySelfTestHumanoid.glb --rig humanoid --atlas creatures/shamwaySelfTestHumanoid.atlas.json --hide bundle/shamwaySelfTestHumanoid_albedo.png --coat tan --seed 7 --size 256` | humanoid rig, biped Walk with opposite-arm swing, tan hide | `bundle/shamwaySelfTestHumanoid.glb` + `_albedo.png` + `.anim.json` | not yet — the `--look` run is the owed picture |
